...
export async function GET(req: NextRequest): Promise<Response | ImageResponse> {
  try {
    ...
    return new ImageResponse(
      (
        <div
          tw={`h-full w-full px-20 py-16 bg-slate-50 border border-slate-100 flex flex-col justify-between`}
        ...
        >
          ...
          <div tw="w-full flex flex-row items-center">
            {avatar?.startsWith("http") ? (
              <img
                src={avatar}
                tw={`mr-4 h-14 w-14 bg-slate-300 rounded-full`}
              />
            ) : (
              <span tw="mr-4 text-5xl">{avatar}</span>
            )}
            <span
              tw={`text-5xl text-gray-600 mr-auto`}
              style={{ fontFamily: "interSemiBold" }}
            >
              {author}
            </span>
